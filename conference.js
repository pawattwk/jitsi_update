...
// console.log('Url_____________:', optioncon.urlhref)
//         if (performance.navigation.type == performance.navigation.TYPE_RELOAD) {
//             window.location.href = optioncon.urlhref
//           } 
import EventEmitter from 'events';
import Logger from 'jitsi-meet-logger';
import attendee from './attendee_join'
import optioncon from './optiononeconference'
import auth_moderator from './current_auth'
 ...
 */
 async function disconnect() {
    ...
    // return connection.disconnect().then(windowLocation.href = 'https://oneconference-new.inet.co.th')
}
...
export default {
    ...
    startConference(con, tracks) {
        ...
        APP.API.notifySubjectChanged('DEMOSE')
        APP.store.dispatch(conferenceSubjectChanged('DEMO'))
    ...
    async init({ roomName }) {
        ...
        // this.roomName = auth_moderator.auth.roomname;
    ...
    _setupListeners() {
        ...
        room.on(JitsiConferenceEvents.KICKED, participant => {
            APP.UI.hideStats();
            // APP.store.dispatch(maybeOpenFeedbackDialog(room)).catch(() => Promise.resolve(false));
            APP.store.dispatch(kickedOut(room, participant)).then(this.hangup(true));
        ...
        // console.log('href____________:' , locationURL.href)
        ...
        if (window.performance) {
            ...
            //   console.log('Reload_________:' , localStorage.getItem("oneurlref"))
            //   console.log('Count__________:' , localStorage.getItem("countreload"))
            //   localStorage.setItem("countreload", localStorage.getItem("countreload") + 1)
            //   if(localStorage.getItem("countreload") == 1){
                window.location.href = localStorage.getItem("oneurlref")
        ...
        APP.UI.addListener(UIEvents.LOGOUT, () => {
            AuthHandler.logout(room).then(url => {
                if (url) {
                    // UIUtil.redirect('https://www.google.com');
                    UIUtil.redirect(url);
                } else {
                    this.hangup(true);
                }
